      ...
      <main className="md:ml-64 p-6 md:p-8 max-w-7xl mx-auto space-y-8 pb-20">
        ...
        <section className="animate-in slide-in-from-bottom-4 duration-500 fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-lg">
                {language === 'en' ? 'Real-time global trend analysis' : 'Анализ глобальных трендов в реальном времени'}
              </p>
            </div>
            <button 
              onClick={refetch}
            ...
            >
              <RefreshCw size={18} className={clsx("text-slate-400 group-hover:text-indigo-500 transition-colors", { "animate-spin text-indigo-500": loading })} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              // Loading Skeletons
              [1, 2, 3].map(i => <TrendCard key={i} loading={true} />)
            ) : (
              trends.map(trend => (
                <SafeTrendCard 
                  key={trend.id}
        ...
        <section className="pt-8 border-t border-slate-200 dark:border-slate-800 animate-in slide-in-from-bottom-8 duration-700 fade-in fill-mode-backwards delay-100">
           <div className="mb-6">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Admin Settings</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm">Protected area for system configuration.</p>
           </div>
